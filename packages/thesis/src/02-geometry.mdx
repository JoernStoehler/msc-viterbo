...
We repeat various known facts about symplectic geometry on polytopes, interpreted as the limit of symplectic geometry on smooth objects that converge to polytopes in some notion (e.g. $C^0$ on the boundary). We restrict ourselves to $\mathbb{R}^4$, the simplest non-trivial case. We restrict ourselves further to convex polytopes, since they are interesting for the Viterbo conjecture, and for simplicity we also demand them to be star-shaped i.e. containing the origin in their interior.
