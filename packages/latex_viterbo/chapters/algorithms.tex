...
\begin{itemize}
  \item A subset \(S \subseteq \{1,\ldots,F\}\) of participating facets, with \(k = |S| \ge 3\).
...
The quadratic form \(Q\) computes (twice the inverse of) the action:
...
The set \(M(S)\) is a compact convex polytope of dimension at most \(k - 5\). It is bounded because \(\beta_i \ge 0\) and \(\sum \beta_i h_i = 1\) with \(h_i > 0\) imply \(\beta_i \le 1/h_i\).
\end{lemma}
...
\begin{proof}
Let \((S^*, \sigma^*, \beta^*)\) achieve the global maximum. If some \(\beta^*_j = 0\), by \cref{lem:hk-boundary}, the same \(Q\) value is achieved by \((S', \sigma', \beta')\) with \(|S'| < |S^*|\). Repeating until no \(\beta_i = 0\) gives a strictly interior maximizer.
\end{proof}
...
The naive algorithm enumerates \(O(F!)\) permutations. For polytopes with sparse facet transition graphs, many permutations can be pruned by checking geometric adjacency (a valid orbit must transition between facets that share a 2-face with compatible Reeb flow direction). The Chaidez--Hutchings bound \(\rho \le 2\) further limits the search to orbits with bounded combinatorial rotation number \cite{CH2021}.
...
\begin{itemize}
  ...
  \item \textbf{No geometric certificate}: The output is a permutation and coefficients, not an explicit trajectory on \(\partial K\).
\end{itemize}
