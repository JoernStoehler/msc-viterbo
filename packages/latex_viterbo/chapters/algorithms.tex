...
\begin{description}
  \item[Facet count] \(n \in \Z_{\ge 5}\) is the number of facets \((F_i)_{i=1}^n\) of \(K\). At least 5 facets are needed to enclose a bounded volume in \(\R^4\).
  ...
  \item[Polytope definition] The polytope is then defined as the intersection of the half-spaces:
    \[
      K = \bigcap_{i=1}^n \{ z \in \R^4 : \inner{n_i}{z} \le h_i \}.
    \]
...
\begin{description}
  \item[Vertex count] \(n_V \in \Z_{\ge 5}\) is the number of vertices of \(K\). No simple correspondence to \(n\) exists in general.
  \item[Vertices] The vertices are points \(v_j \in \R^4\) for \(j \in \{1, \ldots, n_V\}\) such that
    \[
      K = \conv\{ v_j : j = 1, \ldots, n_V \}.
    \]
...
\begin{description}
  \item[Half-space Hyperplane Set] The maximal index set \(I \subset \{1, \ldots, n\}\) of the hyperplanes whose intersection defines the affine subspace containing \(F\). We picked the unique maximum instead of an arbitrary minimal set, to allow us to match data to the \(\ge k+1\)-faces containing \(F\) easily.
  \item[Half-space Inequalities] The maximal index set \(J \subset \{1, \ldots, n\}\) with \(I \subset J\) of sharp half-space inequalities that define \(F\). We picked the sharp inequalities to easily talk about the \(\le k-1\)-faces contained in \(F\). Unsharp inequalities are redundant not just for \(F\) but all sub-faces, and so we omit them.
    \[
      F = \bigcap_{i \in J} \{ z \in \R^4 : \inner{n_i}{z} \le h_i \} \subset \bigcap_{i \in I} \{ z \in \R^4 : \inner{n_i}{z} = h_i \} \cong \R^k.
    \]
  \item[Face Vertices] Indices \(L \subset \{1, \ldots, n_V\}\) of the vertices of \(K\) that are the extreme points of \(F\). We have
    \[
      F = \conv\{ v_l : l \in L \}.
    \]
\end{description}
...
% TODO: Present the first algorithm for capacity computation from the literature.
...
% TODO: Present the third algorithm for capacity computation from the literature.
...
Let \((F_i)_{i=1}^n\) be the set of facets of \(K\). The number of facets is \(n \ge 5\).
...
By \todoref{thm:simple-min-action-reeb-orbit} and \todoref{thm:cz-index} we know that it is enough to consider those Reeb orbits that are simple polygonal, and have rotation \(\leq 2\pi\). In particular, we can ignore Reeb orbits that flow along 1-faces as they have rotation \(\rho = +\infty\).
...
\begin{lemma}
  ...
  \begin{itemize}
    \item The root nodes are all 2-faces \((i_0, i_1)\).
...
\begin{lemma}[Pruning]
  ...
  \begin{itemize}
    ...
    \item If the tube has rotation \(\rho > 2\pi\), then all tubes in the subtree rooted at that node have rotation \(> 2\pi\) as well.
...
\begin{theorem}[Iterative Tube Data Computation]
  ...
  \begin{description}
    ...
    \item[Rotation] Let \(\rho \in \R_{\ge 0}\) be the total rotation of all the trajectories. To avoid definitional issues, we take the rotation of infinitesimally shortened trajectory \(\gamma|_{[\varepsilon, T-\varepsilon]}\).
    \item[Flow map] Let \(\psi: P_{start} \cong P_{end}\) be the flow map that maps any start point to the corresponding endpoint of the trajectory.
  \end{description}
  ...
  \end{align*}
  describing the flow along the facet \(F_{i_{k+1}}\), namely the map between 2-faces, and the associated duration/action increment. And the rotation increment at the new break point:
  \[
    \rho(F_{i_k} \cap F_{i_{k+1}}).
  \]
  Then:
  \begin{align*}
